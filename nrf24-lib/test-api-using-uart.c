...
int main(void)
{
  ...
  uint8_t wid_addr        = rf24_read_config(&rf_handle, SET_ADDR_WID);
  uint8_t freq_cha        = rf24_read_config(&rf_handle, SET_FREQ_CHA);
  uint8_t set_reg         = rf24_read_config(&rf_handle, SET_REG_RATE);
  uint8_t config_data     = rf24_read_config(&rf_handle, CONFIG_REG);

  // TX_ADDR is multi-byte
  ...
  uint8_t wid_addr1        = rf24_read_config(&rf_handle, SET_ADDR_WID);
  uint8_t freq_cha1        = rf24_read_config(&rf_handle, SET_FREQ_CHA);
  uint8_t set_reg1         = rf24_read_config(&rf_handle, SET_REG_RATE);
  uint8_t config_data1     = rf24_read_config(&rf_handle, CONFIG_REG);

  uint8_t tx_after[5] = {0};
  rf24_read_multi(&rf_handle, TX_ADDR, tx_after, rf_handle.addr_len);

  print_reg("SETUP_AW", wid_addr1);
  print_reg("RF_CH", freq_cha1);
  print_reg("RF_SETUP", set_reg1);
  print_reg("CONFIG", config_data1);
